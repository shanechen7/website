...
async function trackContainer({ billNo, containerNo, carrierCode, businessNo }) {
    ...
    if (!json) {
        return { ok: false, message: '飞驼无响应' };
    }
    // 20000 = 订阅成功且有数据；20001 = 订阅成功但数据还在抓取中（首次订阅常见）
    if (json.statusCode !== 20000 && json.statusCode !== 20001) {
        return { ok: false, statusCode: json.statusCode, message: json.message };
    }

    const r = (json.data && json.data.result) || null;
    if (!r) {
        // 首次订阅：飞驼后台正在去船司抓数据，需要稍后再次查询（不会重复扣费）
        return {
            ok: false,
            pending: true,
            statusCode: json.statusCode,
            message: '订阅成功，数据抓取中，请稍后再查',
        };
    }
    ...
    return {
        ...
        containers: containers.map((c) => ({
            ...
            statusEn: c.currentStatusDescriptionEn || '',
        ...
        statusTextEn: cs.descriptionEn || r.statusDescriptionEn || '',
